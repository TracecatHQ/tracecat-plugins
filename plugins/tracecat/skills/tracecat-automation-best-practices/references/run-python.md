...
## Python is the bottom of the escalation ladder

`run_python` is the last rung, not the default. Work down the ladder — inline expression,
`run_if`, `core.transform.*`, then Python — and stop at the first rung that genuinely does the
job. [expressions-and-conditions](expressions-and-conditions.md) has the full ladder, the
expression grammar, and the thirteen transform actions authors most often skip past.
...
A script that returns a boolean, renames three fields, or extracts a handful of JSONPaths is
Python doing an expression's or a transform's job: it costs a sandbox start and hides the logic
from graph review.
