...
| Upload or update a local skill | `list_skills`, then `prepare_skill_upload`, HTTP `PUT` of each file, then `complete_skill_upload` | Metadata only in tool arguments; pass `skill_id` to replace an existing draft, omit it to create — a name collision suffixes the slug rather than failing |
| Read a skill's current files | `get_skill`, or `prepare_skill_download` for the whole draft | `get_skill` without `path` returns the manifest only; never pull file bytes into context |
